...
from transformers import AutoModelForCausalLM, AutoImageProcessor, AutoProcessor, AutoConfig, AutoTokenizer
...
def build_state_initializer(ov_model: ov.Model, batch_dim: int):
    ...
    for op in ov_model.get_ops():
        if op.get_type_name() == "ReadValue":
            ...
            dims = [(opset13.constant(np.array([dim], dtype=np.int64))
                     if isinstance(dim, int) else dim) for dim in dims]
...
def patch_stateful(ov_model):
    key_value_input_names = [key.get_any_name()
                             for key in ov_model.inputs[2:-1]]
    key_value_output_names = [key.get_any_name()
                              for key in ov_model.outputs[1:]]
...
def convert_glmv_model(model_id, output_dir, quantization_config):
    ...
    if not image_embed_path.exists():
        ...
        ov_model = ov.convert_model(model.model.vision, example_input=torch.ones(
    ...
    if not lang_model_path.exists():
        ...
        inputs_embeds = torch.zeros(
            [2, 2, config.hidden_size], dtype=torch.float32)
...
class OvGLMv(GenerationMixin):
    def __init__(self, model_dir, device):
        ...
        self.vision = core.compile_model(
            model_dir / "openvino_vision.xml", "CPU")
        self.embedding = core.compile_model(
            model_dir / "openvino_embedding.xml", "CPU")
        self.input_names = {key.get_any_name(): idx for idx,
                            key in enumerate(self.model.inputs)}
        self.output_names = {key.get_any_name(): idx for idx,
                             key in enumerate(self.model.outputs)}
    ...
    def forward(
        ...
        if not past_key_values:
            ...
            new_input_embeds = []
            multi_flags = [True if self.config.boi_token_id in input_id.tolist(
            ) else False for input_id in input_ids]
            images_features = None
            if not is_empty(pixel_values):
                images_features = torch.from_numpy(
                    self.vision(pixel_values)[0])
            image_count = 0
            for i in range(len(input_ids)):
                ...
                if multi_flags[i]:
                    ...
                    num_image_padding_tokens = input_id.count(
                    ...
                    new_input_embeds.append(
                        torch.cat(
                            (
                                ...
                                images_features[image_count].to(
        ...
        if "beam_idx" in self.input_names:
            inputs["beam_idx"] = self.next_beam_idx if self.next_beam_idx is not None else np.arange(
                inputs_embeds.shape[0], dtype=int)
    ...
    def _reorder_cache(self, past_key_values: Tuple[Tuple[torch.Tensor]], beam_idx: torch.Tensor) -> Tuple[Tuple[torch.Tensor]]:
        ...
        self.next_beam_idx = np.array(
            beam_idx)  # save beam_idx to be used as an input in the next iteration
    ...
    def prepare_inputs_for_generation(
        ...
        pixel_values: Optional[torch.Tensor] = torch.zeros(
            [1, 1, 1, 3, 672, 672]),
    ...
    def _create_position_ids_from_attention_mask(self, attention_mask):
        ...
        for i, mask in enumerate(attention_mask):
            ...
            positions = torch.nonzero(mask, as_tuple=False).squeeze(
                1).to(attention_mask.device)
